<div class="row">
  <div class="col-md-6">
    ...
    <div class="form-group row">
      ...
      <div class="col-sm-8">
        <input type="text" value="{{ old('telepon', $admin->telepon) }}" class="form-control @error('telepon') is-invalid @enderror " name="telepon" placeholder="Masukkan telepon">
        @error('telepon')
        <span class="invalid-feedback mt-1">
          {{ $message }}
        </span>
    ...
    <div class="form-group row">
      ...
      <div class="col-sm-8">
        <select class="form-select form-control @error('is_aktif') is-invalid @enderror" name="is_aktif" id="">
          ...
          <option value="1" {{ $admin->user->is_aktif == '1' ? 'selected' : '' }}>Aktif</option>
          <option value="0" {{ $admin->user->is_aktif == '0' ? 'selected' : '' }}>Non-Aktif</option>
        </select>
        @error('is_aktif')
        <span class="invalid-feedback mt-1">
          {{ $message }}
    ...
    <div class="form-group row">
      ...
      <div class="col-sm-8">
          <input type="text" value="{{ old('username', $admin->user->username) }}"
